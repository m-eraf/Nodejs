...
router.post('/register', function(req, res, next) {
  var sql = "INSERT INTO `putgf` (`name`, `email`,`password`,`section`,`batch`) \
...
router.post('/send', function(req, res, next) {
  var sql = "INSERT INTO `putgfff` (`username`, `messages`) \
  VALUES ('"+req.body.username+"', '"+req.body.mess+"');"
  con.getConnection(function (err, connection) {
    connection.query(sql, function (err, result) {
      if (err) throw err;
      console.log(result);
...
router.post('/auth', function(req, res) {
  ...
  con.getConnection(function (err, connection) {
    if(username=='harsh'&&password==1234){
      res.render('harsh')
		...
		connection.query('SELECT * FROM putgf WHERE name = ? AND password = ?', [username, password], function(error, results, fields) {
  ...
  router.get('/table', function (req, res) {
    con.getConnection(function (err, connection) {
      // if (err) throw err;
      connection.query("SELECT * FROM putgf", function (err, data, fields) {
        // if (err) throw err;
        console.log(data);
  ...
  router.post('/see', function (req, res) {
    con.getConnection(function (err, connection) {
      // if (err) throw err;
      connection.query("SELECT * FROM putgfff", function (err, data, fields) {
        // if (err) throw err;  
        console.log(data);
        res.render('see', { title: 'Messages', userData: data});
      });
    });
  });
